 ...
 */
extern	int athp_dma_head_alloc(struct athp_softc *sc,
	    struct athp_dma_head *dh, int buf_size);
extern	void athp_dma_head_free(struct athp_softc *sc,
	    struct athp_dma_head *dh);

/*
 * Operations on loading/unloading mbufs.
 */
extern	int athp_dma_mbuf_load(struct athp_softc *sc,
	    struct athp_dma_head *dh,
...
extern	void athp_dma_mbuf_post_xmit(struct athp_softc *sc,
	    struct athp_dma_head *dh,
	    struct athp_dma_mbuf *dm);
/* Call before an mbuf is handed to the hardware for receive. */
extern	void athp_dma_mbuf_pre_recv(struct athp_softc *sc,
	    struct athp_dma_head *dh,
	    struct athp_dma_mbuf *dm);
/* Call after an mbuf is handed to the hardware for receive. */
extern	void athp_dma_mbuf_post_recv(struct athp_softc *sc,
	    struct athp_dma_head *dh,
	    struct athp_dma_mbuf *dm);
